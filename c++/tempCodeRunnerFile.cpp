...
int fun(int i, int mask) {
	...
	// we have to travel all nodes j in mask and end the path at ith node so for every node j in mask, recursively calculate cost of travelling all nodes in mask except i and then travel back from node j to node i taking the shortest path take the minimum of all possible j nodes
